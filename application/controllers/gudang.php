...
Class Gudang extends CI_Controller{
	...
	public function index()
	{
		...
		$this->template->gudang("pages/beranda",$data);
	}
	...
	public function profile()
	{
			...
			$data['listcus']  = $this->mweb->listcus($idu);
	
			$this->template->gudang("pages/profile",$data);
	}
			...
			$cekact       = cekurlid($jabatan,'idact');
     		...
     		if ((empty($m)) || (empty($page)) || (empty($slug)) || (strlen($m)===0) || (strlen($page)===0) || (strlen($slug)===0)) {
                ...
                $data['data']=$dataq->result_array();
     			...
     			$this->template->gudang("pages/".$page,$data);
	...
	*/
	...
	function edit($idnya='',$idmng='',$kolid='')
	{
		...
		if($this->session->userdata('logged_in'))
		{
			...
			$this->template->gudang("pages/formedit",$data);
			//END: ada--edit---------------------
	     	}else
	     	{
	     			echo('ga ada');
		...
		}else
		{
			$cekmng = cekurlid('manage','idmng');
     		...
     		}else
     		{
				...
				$this->template->gudang("pages/invoice",$data);
				//END: buat-invoice     					
	     	}else
	     	{
    	 		echo('tidak ada');
		...
		}else
		{
    		$cekmng=cekurlid('barang','kdbarang');
     		...
     		}else if (!in_array($id, $cekmng))
     		{
				...
				$this->template->gudang("pages/setjual",$data);
				//END: set-harga-jual
     		}
		}
    }
